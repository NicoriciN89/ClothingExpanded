...
namespace Toolbelts
{
    internal class TBFunctionalities
    {
        ...
        internal static string attachBeltText = string.Empty;
        private static GameObject? attachBeltButton;
        internal static GearItem? beltItem;
        internal static string beltName = string.Empty;

        internal static string detachBeltText = string.Empty;
        private static GameObject? detachBeltButton;
        internal static GearItem? pantsItem;
        internal static string pantsName = string.Empty;

        internal static string attachCramponsText = string.Empty;
        private static GameObject? attachCramponsButton;
        internal static GearItem? cramponItem;
        internal static string cramponName = string.Empty;

        internal static string detachCramponsText = string.Empty;
        private static GameObject? detachCramponsButton;
        internal static GearItem? bootsItem;
        internal static string bootsName = string.Empty;

        internal static string attachScabbardText = string.Empty;
        private static GameObject? attachScabbardButton;
        internal static GearItem? scabbardItem;

        internal static string detachScabbardText = string.Empty;
        ...
        internal static void InitializeMTB(ItemDescriptionPage itemDescriptionPage)
        {
            ...
            attachBeltButton     = CreateButton(equipButton, attachBeltText,     OnAttachBelt);
            detachBeltButton     = CreateButton(equipButton, detachBeltText,     OnDetachBelt);
            attachCramponsButton = CreateButton(equipButton, attachCramponsText, OnAttachCrampons);
            detachCramponsButton = CreateButton(equipButton, detachCramponsText, OnDetachCrampons);
            attachScabbardButton = CreateButton(equipButton, attachScabbardText, OnAttachScabbard);
            detachScabbardButton = CreateButton(equipButton, detachScabbardText, OnDetachScabbard);
        ...
        private static GameObject CreateButton(GameObject template, string label, System.Action onClick)
        {
            var btn = UnityEngine.Object.Instantiate<GameObject>(template, template.transform.parent, true);
            btn.transform.Translate(0, 0, 0);
            Utils.GetComponentInChildren<UILabel>(btn).text = label;
            var list = new Il2CppSystem.Collections.Generic.List<EventDelegate>();
            list.Add(new EventDelegate(onClick));
            Utils.GetComponentInChildren<UIButton>(btn).onClick = list;
            return btn;
        }

        internal static void SetAttachBeltActive(bool active)
        {
            if (attachBeltButton != null) NGUITools.SetActive(attachBeltButton, active);
        }

        internal static void SetDetachBeltActive(bool active)
        {
            if (detachBeltButton != null) NGUITools.SetActive(detachBeltButton, active);
        }

        internal static void SetAttachCramponsActive(bool active)
        {
            if (attachCramponsButton != null) NGUITools.SetActive(attachCramponsButton, active);
        }

        internal static void SetDetachCramponsActive(bool active)
        {
            if (detachCramponsButton != null) NGUITools.SetActive(detachCramponsButton, active);
        }

        internal static void SetAttachScabbardActive(bool active)
        {
            if (attachScabbardButton != null) NGUITools.SetActive(attachScabbardButton, active);
        }

        internal static void SetDetachScabbardActive(bool active)
        {
            if (detachScabbardButton != null) NGUITools.SetActive(detachScabbardButton, active);
        }
